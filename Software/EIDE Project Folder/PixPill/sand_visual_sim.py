...
for i in random.sample(range(NUM_LEDS), SAND_NUM):
    sand_now[i] = 1
...
def update_sand(ax_tilt, ay_tilt):
    ...
    for i in range(NUM_LEDS):
        ...
        if is_diag:
            ...
            if main_nbr < 0:
                # Wall in diagonal direction → try orthogonal side slides
...
def update_anim(_):
    ...
    ax_tilt = math.sin(frame * 0.05)
    ay_tilt = math.cos(frame * 0.05)
